...
{
	motor.param.commutation_current=5000; //找磁场零位电流，2000表示2A，用于给到Q轴目标电流对齐磁场的。
	motor.motion.commutation_mode=1;					//找磁场零位模式，0表示抖动找零位，1表示通过单圈绝对值位置找零位
	motor.motion.commutation_time=2000;			//抖动找零位时的保持时间，1000表示1秒
...
{
	//operation_mode=1;      //工作模式：1表示有加减速的位置模式，2表示速度模式，3表示有加减速的速度模式，4表示力矩模式，0表示开环速度模式
	//control_word.all=0x06;  //控制字：0x06表示去使能；0x0F表示使能；0x86表示清错
	motor.motion.Iq_demand=0;          	//内部Q轴电流指令，单位是mA，只读
	motor.motion.speed_demand=0;					//内部速度指令，单位是0.001转每秒，只读
	motor.motion.position_demand=0;			//内部位置指令，单位是count，只读
