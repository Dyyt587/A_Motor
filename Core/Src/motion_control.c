...
{
	float temp;
	if (motor.wkc.lic_aprove.bits.position_mode)
	{
		sprintf(display_buff, "Position Mode   ");
	}
	else if (motor.wkc.lic_aprove.bits.velocity_mode)
	{
		sprintf(display_buff, "Velocity Mode   ");
	}
	else if (motor.wkc.lic_aprove.bits.torque_mode)
	{
	...
	else
	{
		...
		// 	break;
		// }
		sprintf(display_buff1, "%s%d%s", "Posact  :", pos_actual, "       ");
		sprintf(display_buff1, "%s%d%s", "Pos  :", pos_actual, "      ");
		/*
		else
		{
	...
	sprintf(display_buff2, "%s%lld%s", "R:", motor.phase_resistor_moh, "");
	extern int Vd;
	...
	sprintf(display_buff3, "%s%d%s", "L:", motor.param.inductance, "muh");
